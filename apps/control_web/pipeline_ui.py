...
@pipeline_ui_bp.errorhandler(
    PipelineApiError
)
def handle_pipeline_api_error(
    exc: PipelineApiError,
):
    body: dict[str, Any] = {
        "status": "ERROR",
        "error": str(
...
def get_pipeline_state():
    with database_read() as connection:
        ...
        task_entities = read_task_entities(
            connection
        )
...
def update_pipeline_config():
    ...
    authorization = tasks.get(
        "authorization"
    )

    export_upd = tasks.get(
        "export_upd"
    )
    ...
    if (
        authorization_enabled
        and not authorization_ids
    ):
        raise PipelineApiError(
            "Для авторизации выберите хотя бы одну организацию.",
            field="tasks.authorization.entity_ids",
        )
    ...
    if export_enabled:
        ...
        if not export_ids:
            raise PipelineApiError(
                ...
                field="tasks.export_upd.entity_ids",
            )
        ...
        unauthorized_export_ids = sorted(
            set(
                export_ids
            )
            - set(
        ...
        if unauthorized_export_ids:
            raise PipelineApiError(
                (
                    "Экспорт УПД можно включить только "
                    "для организаций, участвующих "
                    "в авторизации."
                ),
                field="tasks.export_upd.entity_ids",
            )
...
def request_pipeline_test():
    with database_transaction() as connection:
        ...
        if (
            ...
            raise PipelineApiError(
                (
                    "Другой запуск уже ожидает "
                    "выполнения или выполняется."
                ),
                409,
            )
        ...
        if not authorization_enabled:
            raise PipelineApiError(
                (
                    "В конфигурации не включена "
                    "авторизация. Тестовый запуск "
                    "не содержит выполняемых функций."
                ),
                400,
        ...
        if export_enabled:
            ...
            if unauthorized_export_ids:
                raise PipelineApiError(
                    (
                        "В экспорт включены организации, "
                        "для которых не включена "
                        "авторизация."
                    ),
                    400,
                    field="tasks.export_upd.entity_ids",
